...
export class true1517953427089 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query("INSERT INTO `user` (`uuid`, `email`, `username`, `display_name`, `salt`, `password`, `activation_code`, `roles`, `is_active`, `created_at`, `updated_at`) VALUES ('"+uuid+"', '"+adminMail+"', '"+adminUsername+"', 'Administrator', '"+adminSalt+"', '"+adminPasswordHashed+"', '"+activationCode+"', 'ROLE_SUPER_ADMIN,ROLE_ADMIN', 1, '"+now+"', '"+now+"')");
    }
